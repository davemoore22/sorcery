...
// Overload [] operator - remember if this is a pointer, use the (*options)[i] syntax to deference
// properly!
...
auto Sorcery::Config::get(const std::string &section, const std::string &value) const
	-> std::string {
...
auto Sorcery::Config::_load() -> bool {
	...
	option =
		_settings->GetValue("Gameplay", CSTR(GAME_BATCH_HEALING_AFTER_RETURN_TO_CASTLE), CSTR(ON));
...
bool Sorcery::Config::save() {
	...
	_settings->SetValue(
		"Options", CSTR(OPT_STRICT_MODE), BOOL2OPTIONCSTR(_options[Enums::Options::STRICT_MODE]));
	_settings->SetValue(
		"Options", CSTR(OPT_AUTO_SAVE), BOOL2OPTIONCSTR(_options[Enums::Options::AUTO_SAVE]));
	_settings->SetValue(
		"Options", CSTR(OPT_CHEAT_MODE), BOOL2OPTIONCSTR(_options[Enums::Options::CHEAT_MODE]));
	_settings->SetValue(
		"Options", CSTR(OPT_DICE_ROLLS), BOOL2OPTIONCSTR(_options[Enums::Options::DICE_ROLLS]));
	_settings->SetValue(
		"Graphics", CSTR(GFX_WIREFRAME), BOOL2OPTIONCSTR(_options[Enums::Options::WIREFRAME_MODE]));
	_settings->SetValue("Graphics", CSTR(GFX_TEXTURES),
		BOOL2OPTIONCSTR(_options[Enums::Options::DISPLAY_TEXTURES]));
	_settings->SetValue(
		"Graphics", CSTR(GFX_TRAPS), BOOL2OPTIONCSTR(_options[Enums::Options::DISPLAY_TRAPS]));
