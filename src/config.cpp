...
Sorcery::Config::Config(
	CSimpleIniA *settings, const std::filesystem::path config_file_path)
...
auto Sorcery::Config::_load() -> bool {
	...
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_ALLOW_MIXED_ALIGNMENT_PARTY), CSTR(ON));
	_options[Enums::Options::ALLOW_MIXED_ALIGNMENT_PARTY] =
		option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_STAT_LOSS_ON_LEVEL_UP), CSTR(OFF));
	_options[Enums::Options::STAT_LOSS_ON_LEVEL_UP] = option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_REROLL_HIT_POINTS_ON_LEVEL_GAIN), CSTR(OFF));
	_options[Enums::Options::REROLL_HIT_POINTS_ON_LEVEL_GAIN] =
		option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_STAT_RESET_ON_CLASS_CHANGE), CSTR(OFF));
	_options[Enums::Options::STAT_RESET_ON_CLASS_CHANGE] =
		option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_AGING_ON_CLASS_CHANGE), CSTR(OFF));
	_options[Enums::Options::AGING_ON_CLASS_CHANGE] = option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_ALLOW_RANGED_WEAPONS), CSTR(ON));
	_options[Enums::Options::ALLOW_RANGED_WEAPONS] = option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_SPELLCASTING_IN_SURPRISE_ROUND), CSTR(ON));
	_options[Enums::Options::SPELLCASTING_IN_SURPRISE_ROUND] =
		option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_BATCH_HEALING_AFTER_RETURN_TO_CASTLE), CSTR(ON));
	_options[Enums::Options::BATCH_HEALING_AFTER_RETURN_TO_CASTLE] =
		option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_REROLL_ONES_ON_DICE), CSTR(ON));
	_options[Enums::Options::REROLL_ONES_ON_DICE] = option.compare(ON) == 0;
	option =
		_settings->GetValue("Gameplay", CSTR(GAME_PROTECT_TELEPORT), CSTR(ON));
	_options[Enums::Options::PROTECT_TELEPORT] = option.compare(ON) == 0;

	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_ALLOW_LOST_LEGATED), CSTR(ON));
	_options[Enums::Options::ALLOW_LOST_LEGATED] = option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_ALLOW_CURABLE_DRAIN), CSTR(ON));
	_options[Enums::Options::ALLOW_CURABLE_DRAIN] = option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_ALLOW_SHARED_INVENTORY), CSTR(ON));
	_options[Enums::Options::ALLOW_SHARED_INVENTORY] = option.compare(ON) == 0;
	option = _settings->GetValue(
		"Gameplay", CSTR(GAME_CAMPING_RESTORE_SPELL_POINTS), CSTR(ON));
	_options[Enums::Options::CAMPING_RESTORE_SPELL_POINTS] =
		option.compare(ON) == 0;
	option =
		_settings->GetValue("Gameplay", CSTR(GAME_CHARGED_ITEMS), CSTR(ON));
...
bool Sorcery::Config::save() {
	...
	_settings->SetValue("Options", CSTR(OPT_STRICT_MODE),
		BOOL2OPTIONCSTR(_options[Enums::Options::STRICT_MODE]));
	_settings->SetValue("Options", CSTR(OPT_AUTO_SAVE),
		BOOL2OPTIONCSTR(_options[Enums::Options::AUTO_SAVE]));
	_settings->SetValue("Options", CSTR(OPT_CHEAT_MODE),
		BOOL2OPTIONCSTR(_options[Enums::Options::CHEAT_MODE]));
	_settings->SetValue("Options", CSTR(OPT_DICE_ROLLS),
		BOOL2OPTIONCSTR(_options[Enums::Options::DICE_ROLLS]));
	_settings->SetValue("Graphics", CSTR(GFX_WIREFRAME),
		BOOL2OPTIONCSTR(_options[Enums::Options::WIREFRAME_MODE]));
	_settings->SetValue("Graphics", CSTR(GFX_TEXTURES),
		BOOL2OPTIONCSTR(_options[Enums::Options::DISPLAY_TEXTURES]));
	_settings->SetValue("Graphics", CSTR(GFX_TRAPS),
		BOOL2OPTIONCSTR(_options[Enums::Options::DISPLAY_TRAPS]));
	...
	_settings->SetValue("Graphics", CSTR(GFX_PROGRESS),
		BOOL2OPTIONCSTR(_options[Enums::Options::DISPLAY_PROGRESS]));
	...
	_settings->SetValue("Gameplay", CSTR(GAME_SPELLCASTING_IN_SURPRISE_ROUND),
		BOOL2OPTIONCSTR(
			_options[Enums::Options::SPELLCASTING_IN_SURPRISE_ROUND]));
	_settings->SetValue("Gameplay",
		CSTR(GAME_BATCH_HEALING_AFTER_RETURN_TO_CASTLE),
		BOOL2OPTIONCSTR(
			_options[Enums::Options::BATCH_HEALING_AFTER_RETURN_TO_CASTLE]));
...
auto Sorcery::Config::set_rec_mode() -> void {
	...
	std::array<bool, NUM_GAME_SETTINGS> _rec_settings{true, false, true, false,
...
auto Sorcery::Config::set_strict_mode() -> void {
	...
	std::array<bool, NUM_GAME_SETTINGS> _strict_settings{false, true, true,
