...
auto Sorcery::Input::display([[maybe_unused]] bool &is_yes) -> void {
	...
	with_PopupModal(CSTR(_id), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove) {
		...
		_ctx.ui->draw_frame(
			p_min, p_max,
			ImVec4{_ctx.ui->ui_colour.x, _ctx.ui->ui_colour.y, _ctx.ui->ui_colour.z, _ctx.animation->fade}, rounding);

		const auto title{_ctx.get_string(_component.string_key)};
		auto centre_x{(((_width + 4) / 2) - (title.length() / 2)) * _ctx.ui->metrics->grid_sz()};
		_ctx.ui->draw_text(_ctx.get_string(_component.string_key), ImVec4{1.0f, 1.0f, 1.0f, _ctx.animation->fade},
						   ImVec2{centre_x, _ctx.ui->metrics->grid_sz()}, _font);

		auto input_y{3 * _ctx.ui->metrics->grid_sz()};
		auto input_x{(((_width + 4) / 2) - (_input_width / 2)) * _ctx.ui->metrics->grid_sz()};
		ImGui::SetCursorPos(ImVec2{input_x, input_y});
		with_ItemWidth(ImGui::GetFontSize() * _input_width) {
			ImGui::InputText("##input_text", &_input, ImGuiInputTextFlags_CharsDecimal);
		}
		...
		ImGui::SetCursorPos(ImVec2{centre - (btn_size.x / 2), _ctx.ui->metrics->grid_sz() * 5});
		if (ImGui::Button(ok_lbl.c_str(), btn_size)) {
			...
			if (_input.length() == 0) {
				...
				if (_component.name == "input_donate") {
					...
					auto &character{_game->characters.at(_ctx.controller->get_character(Enums::CharacterSlot::TITHE))};
					...
					if (amount > character.get_gold()) {
						...
						_ctx.ui->popup_manager->open_dialog("global:notice_not_enough_gold",
						...
						_ctx.ui->popup_manager->open_dialog("global:notice_donated_ok", Enums::Layout::DialogType::OK);
