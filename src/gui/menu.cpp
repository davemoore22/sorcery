...
auto Sorcery::Menu::_load_fixed_items() -> void {
	...
	if (_component->name == "compendium_menu") {
		...
		for (const auto &source : sources)
			_items.emplace_back(
				std::format("{:^{}}", _ctx.get_string(source), _width));
		...
		sources.insert(sources.end(),
					   {"CHARACTER_RACE_HUMAN", "CHARACTER_RACE_ELF",
						"CHARACTER_RACE_DWARF", "CHARACTER_RACE_GNOME",
		...
		_items.emplace_back(
			std::format("{:^{}}", _ctx.get_string("BESTIARY_RETURN"), _width));
		...
		_items.emplace_back(
			std::format("{:^{}}", _ctx.get_string("SPELLBOOK_RETURN"), _width));
		...
		_items.emplace_back(
			std::format("{:^{}}", _ctx.get_string("MUSEUM_RETURN"), _width));
	}
}
