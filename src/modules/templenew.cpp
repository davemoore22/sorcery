...
Sorcery::TempleNew::TempleNew(Context &ctx)
	: Module{ctx} {

	_pay = std::make_unique<PayNew>(_ctx);

	_initialise();
};

auto Sorcery::TempleNew::_initialise() -> bool {
...
auto Sorcery::TempleNew::start() -> int {
	...
	while (true) {
		...
		_ctx.ui->display(Enums::Screen::TEMPLE_NEW, _ctx.game);
...
auto Sorcery::TempleNew::stop() -> int {
