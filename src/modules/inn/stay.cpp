...
auto Sorcery::Stay::start() -> int {
	...
	while (true) {
		...
		constexpr std::array recovery_modes{
			RECOVERY_MODE_FREE,		RECOVERY_MODE_COST_10,	RECOVERY_MODE_COST_50,
			RECOVERY_MODE_COST_200, RECOVERY_MODE_COST_500,
		};
		...
		if (room >= 0 && room < static_cast<int>(recovery_modes.size())) {
			...
			const auto recovery_flags{after_age > before_age ? RECOVERY_BIRTHDAY : 0};
		...
		if (!_ctx.controller->wants(Enums::Screen::STAY) && _ctx.controller->wants(Enums::Screen::INN))
			return BACK_TO_INN;
	}
}
