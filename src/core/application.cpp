...
#include "cereal/archives/binary.hpp"	  // for BinaryInputArchive, Binary...
...
#include "core/define.hpp"				  // for EXPEDITION_GOTO, EXPEDITIO...
...
#include "drawables/define.hpp"			  // for ABORT_GAME, LEAVE_GAME
...
/// @param filename
/// @return
auto Sorcery::Application::save_state_to_binary(const std::filesystem::path &filename) -> bool {
...
/// @param filename
/// @return
auto Sorcery::Application::load_state_from_binary(const std::filesystem::path &filename) -> bool {
...
auto Sorcery::Application::_add_quickstart_party() -> void {
	...
	for (int i = 0; i < 12; i++) {
		...
		case SAMURAI:
			...
			break;
		case MAGE:
			add_item(ROBES, false);
			add_item(DAGGER, false);
			break;
		case PRIEST:
		case BISHOP:
			add_item(ROBES, true);
...
/// @brief Check for a command line parameter
/// @param param
/// @return
