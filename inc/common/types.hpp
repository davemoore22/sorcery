...
struct ConsoleMessage {
		...
		std::string text;
		static inline std::uint64_t s_id{0};
...
struct ShopStock {
		...
		ShopStock()
			: initial_stock{0},
			  current_stock{0},
			  shop_will_buy{false},
			  shop_will_sell{false} {};

		ShopStock(int initial_stock_, int current_stock_, bool shop_will_buy_, bool shop_will_sell_)
			: initial_stock{initial_stock_},
			  current_stock{current_stock_},
			  shop_will_buy{shop_will_buy_},
			  shop_will_sell{shop_will_sell_} {};

		template <class Archive> auto serialize(Archive &archive) -> void {
			archive(initial_stock, current_stock, shop_will_buy, shop_will_sell);
		}
