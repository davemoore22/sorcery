...
inline auto GUID() -> std::string {
	...
	UUID uuid{};
	RPC_CSTR guid{nullptr};

	const auto status{UuidCreate(&uuid)};
	if (status != RPC_S_OK && status != RPC_S_UUID_LOCAL_ONLY)
		return {};

	if (UuidToStringA(&uuid, &guid) != RPC_S_OK)
		return {};

	const std::string result{reinterpret_cast<const char *>(guid)};
...
inline auto PATH2UTF8(const std::filesystem::path &path) -> std::string {
