...
class ItemType {
	...
	public:
		...
		auto get_value() const -> std::uint64_t;
		...
		auto get_can_be_bought() const -> bool;
		auto get_can_be_sold() const -> bool;
		...
		auto set_value(std::uint64_t value) -> void;
		...
		auto set_can_be_sold(const bool value) -> void;
		auto set_can_be_bought(const bool value) -> void;
	...
	private:
		...
		std::uint64_t _value;			  // Price to buy in shop (Sell/Identify Price is half this)
		...
		bool _can_be_bought;							  // Can Player buy this item from Shop
		bool _can_be_sold;								  // Can Player sell this item to Shop
