...
namespace Enums::Monsters {
	...
	// Following the original numbering at
	// http://www.pekori.jp/~emonoya/monster/index.html for the sake of
	// convenience and ease of replicating original item tables
	enum class TypeID {
		BUBBLY_SLIME = 0,
		ORC = 1,
		KOBOLD = 2,
...
namespace Enums::Items {
	...
	// Following the original numbering at
	// http://www.pekori.jp/~emonoya/item/index.html for the sake of convenience
	// and ease of replicating original item tables
	enum class TypeID {
		BROKEN_ITEM = 0,
		LONG_SWORD = 1,
		SHORT_SWORD = 2,
		...
		SHIELD_MINUS_1 = 38,
		AMULET_OF_JEWELS = 39,
		// No, I don't know why this is duplicated either, but its in the
		// original tables
	...
	// From Snarfaru's updated list
	// (https://www.zimlab.com/wizardry/walk/wizardry-1-items-list.htm)
	namespace Effects {

		enum class Defensive {
			NO_DEF_EFFECT = 0,
			// Protection effects mean that if a player is targed by a creature
			// of that type, then there is a 50% chance that the creature will
			// skip its attack (see
			// https://datadrivengamer.blogspot.com/2019/08/the-treasury-of-wizardry.html)
			...
			// Breath Resistances work as follows ("DOBREATH" in the original
			// code): There is a Resistance vs Breath Attacks chance of halving
			// the incoming damage If a character is wearing an item with the
			// appropriate Breath Resistance, then any incoming damage is halved
			// again - so it is possible for a character to suffer only 1/4
			// damage on a successful Saving Throw + Wearing an appropriate item
			RESIST_COLD = 14,
			RESIST_FIRE = 15,
			RESIST_MAGIC = 16,
			...
			RESIST_POISON = 19,
			RESIST_STONING = 20,
			// Immunity to decapitation (how is this different from
			// RESIST_PHYSICAL?)
		...
		enum class Invoke {
			...
			INC_HP = 3,
			INC_STRENGTH = 4,
			// These ones are possible in Wiz 1 but never used according to
			// Snafaru
			AGE_BY_A_YEAR = 5,
			BECOME_LORD = 6,
			BECOME_SAMURAI = 7,
